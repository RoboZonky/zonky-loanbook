...
public final class SummaryPurposeTimelineChart extends AbstractTimelineXYZChart {

    public SummaryPurposeTimelineChart(final Data data) {
        super(data, SummaryPurposeTimelineChart::regionTimeline);
    }

    private static Number convert(final String purpose, final List<DataRow> rows) {
        final long total = rows.size();
        final long part = rows.stream().filter(r -> r.getPurpose().equals(purpose)).count();
    ...
    private static Tuple2<String, Function<List<DataRow>, Number>> convert(final String purpose) {
        return Tuple.of(purpose, data ->  convert(purpose, data));
    }
    ...
    public String getTitle() {
        return "Skladba účelu ǘvěrů v čase";
    }
}
